...
table1 <-
  tibble(
    ...
    "Description" = c("Date of data collected",
                      ...
                      among others",
                      "Non-durable goods are anything that are generally consumed within a short period of time. Examples
                      include food, clothing, cosmetics etc",
                      "Expenditure in food by all Americans in a given time period",
...
We gathered our data from Federal Reserve Economic Data (FRED), an extensive online database housing a multitude of economic time series data, both at the US national and international levels. Utilizing the `fredr` package, we integrated six distinct datasets into one data frame. These datasets were titled:
...
In our multiple linear regression analysis, we estimated an intercept of -$97.40 billion USD. For every $1 billion USD rise in income, food expenditure increased by $20 million USD. Consistent with the simple regression model, the standard deviation remains insignificantly small.

Durable goods expenditure exhibits a negative relationship with food expenditure, with a decrease of $15 million. Similarly, services expenditure sees a decrease of $1 million. Thus, the model suggests that an increase in spending on durable goods and services could lead to a decrease in food expenditure. Conversely, there is a positive relationship between expenditure on nondurable goods and food, with a notable increase of $47 million in food spending for increases in nondurable goods spending. This relationship suggests that these expenditures may be complements of each other. Lastly, we observed no significant relationship between healthcare spending and food expenditure. Intuitively, this makes sense, as both categories encompass essential items and are less susceptible to changes in the economic environment.

Both models exhibit similar adjusted $R^2$ values, yet the second model slightly outperforms the first with a higher value (0.993 vs 0.990), indicating a superior fit. Notably, the root-mean-square error (RSME) for the second model is substantially lower than that of the first (10.36 vs 27.49), indicating better predictive accuracy.
...
In future iterations, a narrower time frame excluding such large-scale global shocks would be more ideal. This would enhance the model's external validity and give us an un-biased estimator for our estimand. 
