...
data |>
  ggplot(aes(x = date)) +
  geom_area(aes(y = healthcare_expenditure, fill = "Healthcare")) +
  #geom_area(aes(y = nondurable_expenditure, fill = "Non-durable goods")) +
  #geom_area(aes(y = durable_expenditure, fill = "Durable goods")) +
  #geom_area(aes(y = services_expenditure, fill = "Services")) +
  labs(
    x = "Date",
    y = "Expenditure by categories"
  ) +
  theme_minimal()
```
...
```{r}
#| label: fig-4
#| fig-cap: Relationship between wing length and w
#| echo: false
#| warning: false
#| message: false
