...
We gathered our data from FRED, the Federal Reserve Economic Data, an extensive online database housing a multitude of economic time series, both at the US national and international levels. Utilizing the `fredr` package, we integrated six distinct datasets into one dataframe. These datatsets were:
...
FRED, rather than directly collecting data, relies on contributions from both public and private organizations to populate its database. When aggregating data through averaging, summing, or end-of-period calculations, FRED typically overlooks missing values, except for the first and last observations of each month [@fred].
